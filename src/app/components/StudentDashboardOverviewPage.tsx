...
import { Calendar as CalendarIcon, RefreshCw, Download, Upload, Plus, MoreHorizontal, Filter, ArrowUpDown, ChevronLeft, ChevronRight, ChevronDown, Search, Check, Columns, Users, Activity, AlertCircle, Clock } from 'lucide-react';
...
interface Student { id: string; studentName: string; country: string; visaIntent: string; progressIndicator: string; status: 'active' | 'inactive'; engagementLevel: string; lastActivity: string; aiInvolvement: boolean; }

export const StudentDashboardOverviewPage: React.FC = () => {
  const [date, setDate] = useState<DateRange | undefined>(undefined);
  const [selected, setSelected] = useState<string[]>([]);
  const [visibleColumns, setVisibleColumns] = useState<string[]>(['id', 'studentName', 'country', 'visaIntent', 'progress', 'status', 'lastActivity']);
  const [rowsPerPage, setRowsPerPage] = useState(10);
  const [showExportDialog, setShowExportDialog] = useState(false);
  const [showImportDialog, setShowImportDialog] = useState(false);

  const items: Student[] = [
    { id: 'STU-001', studentName: 'John Smith', country: 'USA', visaIntent: 'F-1 Student Visa', progressIndicator: '78%', status: 'active', engagementLevel: 'High', lastActivity: '2 hours ago', aiInvolvement: true },
    { id: 'STU-002', studentName: 'Sarah Johnson', country: 'UK', visaIntent: 'Tier 4 Student Visa', progressIndicator: '45%', status: 'active', engagementLevel: 'Medium', lastActivity: '1 day ago', aiInvolvement: true },
    { id: 'STU-003', studentName: 'Michael Chen', country: 'Canada', visaIntent: 'Study Permit', progressIndicator: '92%', status: 'active', engagementLevel: 'High', lastActivity: '30 mins ago', aiInvolvement: true },
    { id: 'STU-004', studentName: 'Emma Wilson', country: 'Australia', visaIntent: 'Subclass 500', progressIndicator: '23%', status: 'inactive', engagementLevel: 'Low', lastActivity: '2 weeks ago', aiInvolvement: false },
  ];

  const metrics = [
    { title: 'Active Students', value: '1,876', icon: Users, bgClass: 'bg-purple-50', colorClass: 'text-purple-600', tooltip: 'Total active students in system' },
    { title: 'Students in Progress', value: '1,234', icon: Activity, bgClass: 'bg-blue-50', colorClass: 'text-blue-600', tooltip: 'Students actively progressing' },
    { title: 'Students Pending Action', value: '143', icon: AlertCircle, bgClass: 'bg-amber-50', colorClass: 'text-amber-600', tooltip: 'Students requiring admin action' },
    { title: 'Recently Active Students', value: '542', icon: Clock, bgClass: 'bg-green-50', colorClass: 'text-green-600', tooltip: 'Students active in last 7 days' }
  ];

  const exportColumns: ExportColumn[] = [{ id: 'studentId', label: 'Student ID', defaultSelected: true }, { id: 'studentName', label: 'Student Name', defaultSelected: true }, { id: 'country', label: 'Country', defaultSelected: true }, { id: 'visaIntent', label: 'Visa Intent', defaultSelected: true }, { id: 'progressIndicator', label: 'Progress', defaultSelected: true }, { id: 'status', label: 'Status', defaultSelected: true }, { id: 'engagementLevel', label: 'Engagement Level', defaultSelected: false }, { id: 'aiInvolvement', label: 'AI Involvement', defaultSelected: false }];
  const importFields: ImportField[] = [{ id: 'studentId', label: 'Student ID', required: false, type: 'text' }, { id: 'studentName', label: 'Student Name', required: true, type: 'text' }, { id: 'country', label: 'Country', required: true, type: 'text' }, { id: 'visaIntent', label: 'Visa Intent', required: true, type: 'text' }, { id: 'status', label: 'Status', required: false, type: 'select', options: ['Active', 'Inactive'] }];
  ...
  return (<TooltipProvider><div className="flex-1 overflow-y-auto p-4 md:p-8 custom-scrollbar-light">
    <div className="hidden md:flex justify-between items-center gap-4 mb-8">
      ...
      <div className="flex items-center gap-3">
        ...
        <button className="flex items-center gap-2 bg-[#0e042f] text-white px-6 h-[50px] rounded-xl shadow-lg shadow-purple-900/20 hover:bg-[#1a0c4a] transition-colors text-[16px] font-medium"><Plus size={20} />Add Student</button>
      </div>
    </div>
    ...
    <div className="hidden md:flex justify-between items-center gap-4 mb-6">
      <div className="relative flex-1"><Search size={20} className="absolute inset-y-0 left-4 my-auto text-[#253154]" /><input type="text" placeholder="Search by student name, reference, or identifier..." className="w-full h-[50px] bg-white rounded-xl border-none shadow-sm pl-12 pr-4 text-[16px] font-medium text-gray-700 placeholder-[#253154] focus:ring-2 focus:ring-purple-100 outline-none" /></div>
    ...
    {selected.length > 0 && (<div className="bg-purple-50 px-6 py-3 flex items-center justify-between text-sm border-b border-purple-100 mb-6 rounded-t-2xl"><span className="text-purple-900 font-medium">{selected.length} student{selected.length !== 1 ? 's' : ''} selected</span><div className="flex gap-3"><button className="text-purple-700 font-bold hover:underline">Mark Reviewed</button><button className="text-purple-700 font-bold hover:underline">Trigger Re-evaluation</button><button className="text-purple-700 font-bold hover:underline">Reset Progress</button><button onClick={() => setSelected([])} className="text-purple-700 font-bold hover:underline">Clear</button></div></div>)}
      ...
      <div className="hidden md:block overflow-auto custom-scrollbar-light max-h-[600px]">
        <table className="w-full">
          <thead><tr className="border-b border-gray-100">
            ...
            {visibleColumns.includes('studentName') && <th className="sticky top-0 z-10 bg-white px-6 py-4 text-left text-[14px] font-bold text-[#253154] tracking-wider uppercase">Student Name</th>}
            ...
            {visibleColumns.includes('visaIntent') && <th className="sticky top-0 z-10 bg-white px-6 py-4 text-left text-[14px] font-bold text-[#253154] tracking-wider uppercase">Visa Intent</th>}
            {visibleColumns.includes('progress') && <th className="sticky top-0 z-10 bg-white px-6 py-4 text-left text-[14px] font-bold text-[#253154] tracking-wider uppercase">Progress</th>}
            ...
            {visibleColumns.includes('lastActivity') && <th className="sticky top-0 z-10 bg-white px-6 py-4 text-left text-[14px] font-bold text-[#253154] tracking-wider uppercase">Last Activity</th>}
          ...
          <tbody className="divide-y divide-gray-50">
            {items.map((item) => (
              <tr key={item.id} className={`hover:bg-gray-50 transition-colors ${selected.includes(item.id) ? 'bg-purple-50/30' : ''}`}>
                ...
                {visibleColumns.includes('studentName') && <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-700">{item.studentName}</td>}
                {visibleColumns.includes('country') && <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-700">{item.country}</td>}
                {visibleColumns.includes('visaIntent') && <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-700">{item.visaIntent}</td>}
                {visibleColumns.includes('progress') && <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-700">{item.progressIndicator}</td>}
                {visibleColumns.includes('status') && <td className="px-6 py-4 whitespace-nowrap text-sm"><StatusBadge status={item.status} /></td>}
                {visibleColumns.includes('lastActivity') && <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-600">{item.lastActivity}</td>}
    ...
    <ExportDialog open={showExportDialog} onOpenChange={setShowExportDialog} moduleName="Student Dashboard" totalCount={1876} selectedCount={selected.length} columns={exportColumns} supportsDateRange={false} onExport={handleExport} />
    <ImportDialog open={showImportDialog} onOpenChange={setShowImportDialog} moduleName="Student Dashboard" fields={importFields} onImport={handleImport} templateUrl="/templates/student-dashboard-import-template.xlsx" allowUpdate={true} />
  </TooltipProvider>);
};