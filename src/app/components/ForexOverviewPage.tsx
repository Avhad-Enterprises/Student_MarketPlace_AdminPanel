...
import { Calendar as CalendarIcon, RefreshCw, Download, Upload, Plus, MoreHorizontal, Filter, ArrowUpDown, ChevronLeft, ChevronRight, ChevronDown, Search, Check, Columns, DollarSign, TrendingDown, Globe, Zap, Eye, Edit } from 'lucide-react';
...
interface Forex { id: string; provider: string; serviceType: string; currencyPairs: number; countries: number; status: 'active' | 'inactive'; studentVisible: boolean; avgFee: string; transferSpeed: string; popularity: number; }
...
export const ForexOverviewPage: React.FC<ForexOverviewPageProps> = ({ onNavigate }) => {
  ...
  const [visibleColumns, setVisibleColumns] = useState<string[]>(['id', 'provider', 'serviceType', 'currencyPairs', 'countries', 'status', 'visible']);
  const [rowsPerPage, setRowsPerPage] = useState(10);
  const [showExportDialog, setShowExportDialog] = useState(false);
  const [showImportDialog, setShowImportDialog] = useState(false);

  const items: Forex[] = [
    { id: 'FRX-1101', provider: 'Wise (TransferWise)', serviceType: 'Money Transfer', currencyPairs: 56, countries: 195, status: 'active', studentVisible: true, avgFee: '0.5%', transferSpeed: '1-2 days', popularity: 8765 },
    { id: 'FRX-1102', provider: 'Revolut', serviceType: 'Multi-Currency Account', currencyPairs: 28, countries: 150, status: 'active', studentVisible: true, avgFee: '0.3%', transferSpeed: 'Instant', popularity: 7654 },
    { id: 'FRX-1103', provider: 'Western Union', serviceType: 'Money Transfer', currencyPairs: 120, countries: 200, status: 'active', studentVisible: true, avgFee: '2.5%', transferSpeed: 'Same day', popularity: 6543 },
    { id: 'FRX-1104', provider: 'XE Money Transfer', serviceType: 'Money Transfer', currencyPairs: 100, countries: 170, status: 'active', studentVisible: false, avgFee: '1.2%', transferSpeed: '1-4 days', popularity: 4321 },
  ];

  const metrics = [
    { title: 'Forex Partners', value: '34', icon: DollarSign, bgClass: 'bg-purple-50', colorClass: 'text-purple-600', tooltip: 'Total forex service partners' },
    { title: 'Currency Pairs', value: '156', icon: Globe, bgClass: 'bg-blue-50', colorClass: 'text-blue-600', tooltip: 'Total currency pairs supported' },
    { title: 'Avg Transfer Fee', value: '1.1%', icon: TrendingDown, bgClass: 'bg-green-50', colorClass: 'text-green-600', tooltip: 'Average transfer fee across providers' },
    { title: 'Instant Transfers', value: '67%', icon: Zap, bgClass: 'bg-amber-50', colorClass: 'text-amber-600', tooltip: 'Percentage of instant transfer options' }
  ];

  const exportColumns: ExportColumn[] = [{ id: 'referenceId', label: 'Reference ID', defaultSelected: true }, { id: 'provider', label: 'Provider', defaultSelected: true }, { id: 'serviceType', label: 'Service Type', defaultSelected: true }, { id: 'currencyPairs', label: 'Currency Pairs', defaultSelected: true }, { id: 'countries', label: 'Countries', defaultSelected: true }, { id: 'status', label: 'Status', defaultSelected: true }];
  const importFields: ImportField[] = [{ id: 'referenceId', label: 'Reference ID', required: false, type: 'text' }, { id: 'provider', label: 'Provider', required: true, type: 'text' }, { id: 'serviceType', label: 'Service Type', required: true, type: 'select', options: ['Money Transfer', 'Multi-Currency Account', 'Currency Exchange'] }, { id: 'currencyPairs', label: 'Currency Pairs', required: true, type: 'text' }, { id: 'status', label: 'Status', required: false, type: 'select', options: ['Active', 'Inactive'] }];
  ...
  return (<TooltipProvider><div className="flex-1 overflow-y-auto p-4 md:p-8 custom-scrollbar-light">
    <div className="hidden md:flex justify-between items-center gap-4 mb-8">
      ...
      <div className="flex items-center gap-3">
        ...
        <button className="flex items-center gap-2 bg-[#0e042f] text-white px-6 h-[50px] rounded-xl shadow-lg shadow-purple-900/20 hover:bg-[#1a0c4a] transition-colors text-[16px] font-medium"><Plus size={20} />Add Forex</button>
      </div>
    </div>
    ...
    <div className="hidden md:flex justify-between items-center gap-4 mb-6">
      <div className="relative flex-1"><Search size={20} className="absolute inset-y-0 left-4 my-auto text-[#253154]" /><input type="text" placeholder="Search forex services..." className="w-full h-[50px] bg-white rounded-xl border-none shadow-sm pl-12 pr-4 text-[16px] font-medium text-gray-700 placeholder-[#253154] focus:ring-2 focus:ring-purple-100 outline-none" /></div>
      ...
      <div className="hidden md:block overflow-auto custom-scrollbar-light max-h-[600px]">
        <table className="w-full">
          <thead><tr className="border-b border-gray-100">
            ...
            {visibleColumns.includes('currencyPairs') && <th className="sticky top-0 z-10 bg-white px-6 py-4 text-left text-[14px] font-bold text-[#253154] tracking-wider uppercase">Currency Pairs</th>}
            ...
            {items.map((item) => (
              <tr
                ...
                onClick={(e) => {
                  ...
                  onNavigate?.('forex-provider-detail');
                }}
              >
                ...
                {visibleColumns.includes('currencyPairs') && <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-700">{item.currencyPairs}</td>}
                ...
                <td className="px-6 py-4 whitespace-nowrap text-sm">
                  <DropdownMenu>
                    ...
                    <DropdownMenuContent className="w-40">
                      <DropdownMenuItem className="flex items-center gap-2" onClick={() => onNavigate?.('forex-provider-detail')}><Eye size={16} className="text-gray-400" />View</DropdownMenuItem>
                      <DropdownMenuItem className="flex items-center gap-2" onClick={() => onNavigate?.('forex-provider-detail')}><Edit size={16} className="text-gray-400" />Edit</DropdownMenuItem>
                    </DropdownMenuContent>
                  </DropdownMenu>
                </td>
              </tr>
    ...
    <ExportDialog open={showExportDialog} onOpenChange={setShowExportDialog} moduleName="Forex" totalCount={34} selectedCount={selected.length} columns={exportColumns} supportsDateRange={false} onExport={handleExport} />
