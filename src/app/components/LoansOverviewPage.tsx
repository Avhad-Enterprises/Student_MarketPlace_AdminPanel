...
import { Calendar as CalendarIcon, RefreshCw, Download, Upload, Plus, MoreHorizontal, Filter, ArrowUpDown, ChevronLeft, ChevronRight, ChevronDown, Search, Edit, Check, Columns, Eye, Power, Globe, Users, CreditCard, Percent, TrendingUp, DollarSign } from 'lucide-react';
...
interface Loan { id: string; provider: string; productName: string; amountRange: string; countries: number; status: 'active' | 'inactive'; studentVisible: boolean; interestType: string; collateral: string; approvalRate: string; popularity: number; lastUpdated: string; }
...
export const LoansOverviewPage: React.FC<{ onNavigate?: (page: string) => void }> = ({ onNavigate }) => {
  ...
  const loans: Loan[] = [
    { id: 'LON-7801', provider: 'Prodigy Finance', productName: 'International Student Loan', amountRange: '$10k-$100k', countries: 150, status: 'active', studentVisible: true, interestType: 'Variable', collateral: 'No', approvalRate: '85%', popularity: 5432, lastUpdated: '1 hour ago' },
    { id: 'LON-7802', provider: 'MPOWER Financing', productName: 'Student Loan Program', amountRange: '$5k-$50k', countries: 195, status: 'active', studentVisible: true, interestType: 'Fixed', collateral: 'No', approvalRate: '78%', popularity: 4321, lastUpdated: '3 hours ago' },
    { id: 'LON-7803', provider: 'Discover Student Loans', productName: 'International Student Loan', amountRange: '$1k-$200k', countries: 1, status: 'active', studentVisible: true, interestType: 'Variable', collateral: 'Yes', approvalRate: '72%', popularity: 3210, lastUpdated: '5 hours ago' },
    { id: 'LON-7804', provider: 'Sallie Mae', productName: 'Private Student Loan', amountRange: '$1k-$150k', countries: 1, status: 'active', studentVisible: false, interestType: 'Fixed', collateral: 'Yes', approvalRate: '68%', popularity: 2109, lastUpdated: '2 days ago' },
    { id: 'LON-7805', provider: 'SoFi', productName: 'Student Loan Refinance', amountRange: '$5k-$500k', countries: 1, status: 'inactive', studentVisible: false, interestType: 'Variable', collateral: 'No', approvalRate: '55%', popularity: 1087, lastUpdated: '1 week ago' },
  ];

  const metrics = [
    { title: 'Loan Providers', value: '28', icon: CreditCard, bgClass: 'bg-purple-50', colorClass: 'text-purple-600', tooltip: 'Total loan provider partners' },
    { title: 'Active Loan Products', value: '87', icon: DollarSign, bgClass: 'bg-blue-50', colorClass: 'text-blue-600', tooltip: 'Currently active loan products' },
    { title: 'Countries Supported', value: '195', icon: Globe, bgClass: 'bg-green-50', colorClass: 'text-green-600', tooltip: 'Countries with loan support' },
    { title: 'Average Approval Rate', value: '76%', icon: TrendingUp, bgClass: 'bg-amber-50', colorClass: 'text-amber-600', tooltip: 'Average loan approval rate' }
  ];
  ...
  const exportColumns: ExportColumn[] = [{ id: 'referenceId', label: 'Reference ID', defaultSelected: true }, { id: 'provider', label: 'Provider', defaultSelected: true }, { id: 'productName', label: 'Product Name', defaultSelected: true }, { id: 'amountRange', label: 'Amount Range', defaultSelected: true }, { id: 'countries', label: 'Countries', defaultSelected: true }, { id: 'status', label: 'Status', defaultSelected: true }, { id: 'interestType', label: 'Interest Type', defaultSelected: false }, { id: 'collateral', label: 'Collateral Requirement', defaultSelected: false }];
  const importFields: ImportField[] = [{ id: 'referenceId', label: 'Reference ID', required: false, type: 'text' }, { id: 'provider', label: 'Provider', required: true, type: 'text' }, { id: 'productName', label: 'Product Name', required: true, type: 'text' }, { id: 'amountRange', label: 'Amount Range', required: true, type: 'text' }, { id: 'countries', label: 'Countries', required: true, type: 'text' }, { id: 'status', label: 'Status', required: false, type: 'select', options: ['Active', 'Inactive'] }, { id: 'interestType', label: 'Interest Type', required: false, type: 'select', options: ['Fixed', 'Variable'] }, { id: 'collateral', label: 'Collateral Requirement', required: false, type: 'select', options: ['Yes', 'No'] }];

  const handleExport = async (options: any) => { toast.success(`Exporting ${options.scope} loan records as ${options.format.toUpperCase()}`); await new Promise(r => setTimeout(r, 2000)); };
  ...
  return (<TooltipProvider><div className="flex-1 overflow-y-auto p-4 md:p-8 custom-scrollbar-light">
    <div className="hidden md:flex justify-between items-center gap-4 mb-8">
      ...
      <div className="flex items-center gap-3">
        ...
        <button className="flex items-center gap-2 bg-[#0e042f] text-white px-6 h-[50px] rounded-xl shadow-lg shadow-purple-900/20 hover:bg-[#1a0c4a] transition-colors text-[16px] font-medium"><Plus size={20} strokeWidth={1.5} />Add Loan</button>
      </div>
    </div>
    ...
    <div className="hidden md:flex justify-between items-center gap-4 mb-6">
      <div className="relative flex-1"><Search size={20} className="absolute inset-y-0 left-4 my-auto text-[#253154]" /><input type="text" placeholder="Search by provider, product name, reference ID" className="w-full h-[50px] bg-white rounded-xl border-none shadow-sm pl-12 pr-4 text-[16px] font-medium text-gray-700 placeholder-[#253154] focus:ring-2 focus:ring-purple-100 outline-none" /></div>
      ...
      <div className="hidden md:block overflow-auto custom-scrollbar-light max-h-[600px]">
        <table className="w-full">
          <thead><tr className="border-b border-gray-100">
            <th className="sticky top-0 z-10 bg-white px-6 py-4 text-left w-12"><CustomCheckbox checked={selectedLoans.length === loans.length} partial={selectedLoans.length > 0 && selectedLoans.length < loans.length} onChange={() => setSelectedLoans(selectedLoans.length === loans.length ? [] : loans.map(l => l.id))} /></th>
              ...
              <tr
                ...
                onClick={(e) => {
                  ...
                  if (!target.closest('td:first-child') && !target.closest('td:last-child')) {
                    onNavigate?.('loan-provider-detail'); // Changed from 'loan-product-detail'
                  }
                }}
                className={`hover:bg-gray-50 transition-colors cursor-pointer ${selectedLoans.includes(loan.id) ? 'bg-purple-50/30' : ''}`}
              >
                <td className="px-6 py-4" onClick={(e) => e.stopPropagation()}><CustomCheckbox checked={selectedLoans.includes(loan.id)} onChange={() => setSelectedLoans(prev => prev.includes(loan.id) ? prev.filter(x => x !== loan.id) : [...prev, loan.id])} /></td>
                {visibleColumns.includes('id') && <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-[#253154]">{loan.id}</td>}
                {visibleColumns.includes('provider') && <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-700">{loan.provider}</td>}
                {visibleColumns.includes('product') && <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-700">{loan.productName}</td>}
                {visibleColumns.includes('amount') && <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-700">{loan.amountRange}</td>}
                {visibleColumns.includes('countries') && <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-700">{loan.countries}</td>}
                {visibleColumns.includes('status') && <td className="px-6 py-4 whitespace-nowrap text-sm"><StatusBadge status={loan.status} /></td>}
                {visibleColumns.includes('visible') && <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-700">{loan.studentVisible ? 'Yes' : 'No'}</td>}
    ...
    <ExportDialog open={showExportDialog} onOpenChange={setShowExportDialog} moduleName="Loans" totalCount={87} selectedCount={selectedLoans.length} columns={exportColumns} supportsDateRange={false} onExport={handleExport} />
