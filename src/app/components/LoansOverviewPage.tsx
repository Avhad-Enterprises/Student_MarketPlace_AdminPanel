            ...
            {loans.length > 0 ? loans.map((loan) => (
              <tr key={loan.id} onClick={(e) => { const target = e.target as HTMLElement; if (!target.closest('td:first-child') && !target.closest('td:last-child')) onNavigate?.('loan-provider-detail'); }} className={`hover:bg-gray-50 transition-colors cursor-pointer ${selectedLoans.includes(loan.id.toString()) ? 'bg-purple-50/30' : ''}`}>
                ...
                <td className="px-6 py-4 whitespace-nowrap text-sm" onClick={(e) => e.stopPropagation()}>
                  <Popover>
                    <PopoverTrigger asChild><button className="p-2 hover:bg-gray-100 rounded-xl transition-colors"><MoreHorizontal size={20} className="text-gray-400" /></button></PopoverTrigger>
                    <PopoverContent className="w-[160px] p-2 bg-white rounded-2xl shadow-xl border border-gray-100" align="end">
                      <div className="flex flex-col gap-1">
                        <button onClick={() => { setEditLoan(loan); setShowAddDialog(true); }} className="w-full flex items-center gap-3 px-4 py-3 text-[14px] font-medium text-gray-600 hover:bg-gray-50 rounded-xl transition-all border border-transparent hover:border-gray-200 active:scale-95 group"><Edit size={18} className="text-gray-400 group-hover:text-gray-600" />Edit</button>
                        <button onClick={() => handleDelete(loan.id)} className="w-full flex items-center gap-3 px-4 py-3 text-[14px] font-medium text-red-500 hover:bg-red-50 rounded-xl transition-all border border-transparent hover:border-red-100 active:scale-95 group"><Trash2 size={18} className="text-red-400 group-hover:text-red-500" />Delete</button>
                      </div>
                    </PopoverContent>
                  </Popover>
                </td>
              </tr>
            )) : (<tr><td colSpan={10} className="px-6 py-20 text-center"><div className="flex flex-col items-center gap-3"><CreditCard size={48} className="text-gray-200" /><p className="text-gray-500 font-medium">No loan products found</p><button onClick={() => { setEditLoan(null); setShowAddDialog(true); }} className="text-purple-600 font-bold hover:underline">Add your first product</button></div></td></tr>)}
          </tbody>
        </table>
      </div>
