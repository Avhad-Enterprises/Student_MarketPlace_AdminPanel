...
import { Calendar as CalendarIcon, RefreshCw, Download, Upload, Plus, MoreHorizontal, Filter, ArrowUpDown, ChevronLeft, ChevronRight, ChevronDown, Search, Check, Columns, UtensilsCrossed, MapPin, Percent, TrendingUp } from 'lucide-react';
...
import { FoodProviderDetail } from './FoodProviderDetail';
...
interface Food { id: string; provider: string; serviceType: string; cuisineTypes: string; countries: number; status: 'active' | 'inactive'; studentVisible: boolean; avgDiscount: string; verified: boolean; popularity: number; }

export const FoodOverviewPage: React.FC = () => {
  const [date, setDate] = useState<DateRange | undefined>(undefined);
  const [selected, setSelected] = useState<string[]>([]);
  const [visibleColumns, setVisibleColumns] = useState<string[]>(['id', 'provider', 'serviceType', 'cuisineTypes', 'countries', 'status', 'visible']);
  ...
  const items: Food[] = [
    { id: 'FOD-1401', provider: 'UberEats Student', serviceType: 'Meal Delivery', cuisineTypes: 'All Types', countries: 45, status: 'active', studentVisible: true, avgDiscount: '15%', verified: true, popularity: 8765 },
    { id: 'FOD-1402', provider: 'DoorDash Campus', serviceType: 'Meal Delivery', cuisineTypes: 'All Types', countries: 4, status: 'active', studentVisible: true, avgDiscount: '12%', verified: true, popularity: 7654 },
    { id: 'FOD-1403', provider: 'GrubHub for Students', serviceType: 'Meal Delivery', cuisineTypes: 'All Types', countries: 3, status: 'active', studentVisible: true, avgDiscount: '10%', verified: true, popularity: 6543 },
    { id: 'FOD-1404', provider: 'HelloFresh Student Box', serviceType: 'Meal Kit', cuisineTypes: 'Various', countries: 15, status: 'active', studentVisible: false, avgDiscount: '20%', verified: false, popularity: 4321 },
  ];

  const metrics = [
    { title: 'Food Partners', value: '27', icon: UtensilsCrossed, bgClass: 'bg-purple-50', colorClass: 'text-purple-600', tooltip: 'Total food service partners' },
    { title: 'Cities Available', value: '450', icon: MapPin, bgClass: 'bg-blue-50', colorClass: 'text-blue-600', tooltip: 'Cities with food service coverage' },
    { title: 'Avg Student Discount', value: '14%', icon: Percent, bgClass: 'bg-green-50', colorClass: 'text-green-600', tooltip: 'Average discount for students' },
    { title: 'Monthly Orders', value: '3.2k', icon: TrendingUp, bgClass: 'bg-amber-50', colorClass: 'text-amber-600', tooltip: 'Average monthly orders by students' }
  ];

  const exportColumns: ExportColumn[] = [{ id: 'referenceId', label: 'Reference ID', defaultSelected: true }, { id: 'provider', label: 'Provider', defaultSelected: true }, { id: 'serviceType', label: 'Service Type', defaultSelected: true }, { id: 'cuisineTypes', label: 'Cuisine Types', defaultSelected: true }, { id: 'countries', label: 'Countries', defaultSelected: true }, { id: 'status', label: 'Status', defaultSelected: true }];
  const importFields: ImportField[] = [{ id: 'referenceId', label: 'Reference ID', required: false, type: 'text' }, { id: 'provider', label: 'Provider', required: true, type: 'text' }, { id: 'serviceType', label: 'Service Type', required: true, type: 'select', options: ['Meal Delivery', 'Meal Kit', 'Catering'] }, { id: 'cuisineTypes', label: 'Cuisine Types', required: true, type: 'text' }, { id: 'status', label: 'Status', required: false, type: 'select', options: ['Active', 'Inactive'] }];
  ...
  const handleRowClick = (providerId: string, event: React.MouseEvent) => {
    // Don't navigate if clicking checkbox or actions button
    ...
    toast.info(`Opening ${providerId} details...`);
  };

  // If a provider is selected, show detail page
  ...
  return (<TooltipProvider><div className="flex-1 overflow-y-auto p-4 md:p-8 custom-scrollbar-light">
    <div className="hidden md:flex justify-between items-center gap-4 mb-8">
      ...
      <div className="flex items-center gap-3">
        ...
        <button className="flex items-center gap-2 bg-[#0e042f] text-white px-6 h-[50px] rounded-xl shadow-lg shadow-purple-900/20 hover:bg-[#1a0c4a] transition-colors text-[16px] font-medium"><Plus size={20} />Add Service</button>
      </div>
    </div>
    ...
    <div className="hidden md:flex justify-between items-center gap-4 mb-6">
      <div className="relative flex-1"><Search size={20} className="absolute inset-y-0 left-4 my-auto text-[#253154]" /><input type="text" placeholder="Search food services..." className="w-full h-[50px] bg-white rounded-xl border-none shadow-sm pl-12 pr-4 text-[16px] font-medium text-gray-700 placeholder-[#253154] focus:ring-2 focus:ring-purple-100 outline-none" /></div>
      ...
      <div className="hidden md:block overflow-auto custom-scrollbar-light max-h-[600px]">
        <table className="w-full">
          <thead><tr className="border-b border-gray-100">
            ...
            {visibleColumns.includes('cuisineTypes') && <th className="sticky top-0 z-10 bg-white px-6 py-4 text-left text-[14px] font-bold text-[#253154] tracking-wider uppercase">Cuisine Types</th>}
          ...
          <tbody className="divide-y divide-gray-50">
            {items.map((item) => (
              <tr key={item.id} className={`hover:bg-gray-50 transition-colors cursor-pointer ${selected.includes(item.id) ? 'bg-purple-50/30' : ''}`} onClick={(e) => handleRowClick(item.id, e)}>
                ...
                {visibleColumns.includes('serviceType') && <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-700">{item.serviceType}</td>}
                {visibleColumns.includes('cuisineTypes') && <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-700">{item.cuisineTypes}</td>}
    ...
    <ExportDialog open={showExportDialog} onOpenChange={setShowExportDialog} moduleName="Food" totalCount={27} selectedCount={selected.length} columns={exportColumns} supportsDateRange={false} onExport={handleExport} />
    <ImportDialog open={showImportDialog} onOpenChange={setShowImportDialog} moduleName="Food" fields={importFields} onImport={handleImport} templateUrl="/templates/food-import-template.xlsx" allowUpdate={true} />
  </TooltipProvider>);
};