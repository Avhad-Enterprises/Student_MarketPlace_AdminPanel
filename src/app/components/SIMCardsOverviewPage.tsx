...
const MobileSIMCard: React.FC<MobileSIMCardProps> = ({ sim, isSelected, onToggleSelect }) => {
  ...
  return (
    <div className={`bg-white rounded-[16px] p-4 shadow-[0_2px_8px_rgba(0,0,0,0.04)] border border-gray-100 w-full transition-all active:scale-[0.99] cursor-pointer flex flex-col gap-2 ${isSelected ? 'bg-purple-50/30' : ''}`}>
      ...
      <div className="flex flex-wrap items-center gap-2">
        <span className="font-bold text-[#253154] text-[15px]">{sim.sim_id}</span>
        <span className="bg-[#F4F4F4] text-gray-500 text-[10px] px-2 py-1 rounded-lg">Tap to view</span>
        <span className="text-sm text-gray-600 ml-auto">{sim.countries_covered} countries</span>
        <button
      ...
      {/* Middle row */}
      <div className="flex flex-wrap items-center gap-2">
        <span className="text-gray-600 text-sm">{sim.provider_name}</span>
      ...
      {isExpanded && (
        <div className="mt-2 pt-3 border-t border-gray-50 animate-in slide-in-from-top-2 fade-in duration-200">
          <div className="grid grid-cols-2 gap-y-3 gap-x-4 mb-4">
            <div>
              <div className="text-[10px] text-gray-400 uppercase tracking-wider font-bold mb-1">Service</div>
              <div className="text-sm text-gray-700 font-medium">{sim.service_name}</div>
            </div>
            <div>
              <div className="text-[10px] text-gray-400 uppercase tracking-wider font-bold mb-1">Network</div>
              <div className="text-sm text-gray-700 font-medium">{sim.network_type}</div>
            </div>
            <div>
              <div className="text-[10px] text-gray-400 uppercase tracking-wider font-bold mb-1">Data</div>
              <div className="text-sm text-gray-700 font-medium">{sim.data_allowance}</div>
            </div>
            <div>
              <div className="text-[10px] text-gray-400 uppercase tracking-wider font-bold mb-1">Validity</div>
              <div className="text-sm text-gray-700 font-medium">{sim.validity}</div>
            </div>
          </div>
              ...
              <tbody className="divide-y divide-gray-50">
                {sims.map((sim) => (
                  <tr
                    key={sim.id}
                    onClick={() => {
                      // Navigate to provider detail page when clicking on Airalo
                      if (sim.provider_name === 'Airalo' && onNavigate) {
                        onNavigate('provider-overview');
                      }
                    }}
                    className={`hover:bg-gray-50 transition-colors group cursor-pointer ${selectedSIMs.includes(sim.id.toString()) ? 'bg-purple-50/30' : ''}`}
                  >
                    ...
                    {visibleColumns.includes('id') && <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-[#253154]">{sim.sim_id}</td>}
                    {visibleColumns.includes('provider') && <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-700">{sim.provider_name}</td>}
                    {visibleColumns.includes('service') && <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-700">{sim.service_name}</td>}
                    {visibleColumns.includes('countries') && <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-700">{sim.countries_covered}</td>}
                    {visibleColumns.includes('status') && <td className="px-6 py-4 whitespace-nowrap text-sm"><StatusBadge status={sim.status} /></td>}
                    {visibleColumns.includes('network') && <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-700">{sim.network_type}</td>}
                    {visibleColumns.includes('data') && <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-700">{sim.data_allowance}</td>}
                    {visibleColumns.includes('validity') && <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-700">{sim.validity}</td>}
                    {visibleColumns.includes('visible') && <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-700">{sim.student_visible ? 'Yes' : 'No'}</td>}
                    {visibleColumns.includes('popularity') && <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-700">{sim.popularity.toLocaleString()}</td>}
                    {visibleColumns.includes('updated') && <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-600">{sim.updated_at ? format(new Date(sim.updated_at), 'MMM dd, yyyy') : 'N/A'}</td>}
                    <td className="px-6 py-4 whitespace-nowrap text-sm">
                      <div className="relative group/menu">
                        <button className="p-1 hover:bg-gray-100 rounded transition-colors">
                          <MoreHorizontal size={18} className="text-gray-400" />
                        </button>
                        <div className="hidden group-hover/menu:block absolute right-0 top-full mt-1 bg-white rounded-xl shadow-xl border border-gray-100 z-20 p-2 w-56">
                          <button
                            onClick={() => onNavigate?.('simcard-provider-detail')}
